...
uses
  ...
  Spr1LabeledEditUnit, SteelMarkUnit, Spr1JvCalcEditUnit, GrMaterialsUnit;
...
TMaterial = class
 ...
 ACTIVE: boolean;
 Procedure Clear;
 Procedure CopyToGrMaterials(Var Gr:TGrMaterials);
 ...
 Function ToForm(Var Form:TMaterialForm;New:boolean):boolean;
 Function FromForm(Var Form:TMaterialForm):boolean;
...
Function TMaterial.GetUnicalFieldValue:string;
Begin
 result:=Name;
End;

Procedure TMaterial.CopyToGrMaterials(Var Gr:TGrMaterials);
Begin
 Gr.CPARENT:=CPARENT;
 Gr.NAME:=NAME;
// ShowMessage('cparent='+IntToStr(CPARENT));
 Gr.PROP_STEELMARK:=PROP_STEELMARK;
 Gr.PROP_THICK:=PROP_THICK;

 if CPARENT<>4 then
 Begin
 Gr.PROP_PROFNUM:=PROP_PROFNUM;
 Gr.PROP_NORMDOC0:=PROP_NORMDOC0;
 Gr.PROP_NORMDOC1:=PROP_NORMDOC1;
 Gr.PROP_LINEDENS:=PROP_LINEDENS;
 Gr.PROP_PWIDTH1:=PROP_PWIDTH1;
 Gr.PROP_PWIDTH2:=PROP_PWIDTH2;
 Gr.PROP_SIZE1:=PROP_SIZE1;
 Gr.PROP_SIZE2:=PROP_SIZE2;
 Gr.PROP_WTHICK:=PROP_WTHICK;
 Gr.PROP_SLENGTH:=PROP_SLENGTH;
 Gr.PROP_PROFTYPE:=PROP_PROFTYPE;
 Gr.PROP_FIXTYPE:=PROP_FIXTYPE;
 Gr.PROP_CARVTYPE:=PROP_CARVTYPE;
 Gr.PROP_DIAM:=PROP_DIAM;
 Gr.PROP_SIZE:=PROP_SIZE;
 Gr.PROP_UW:=PROP_UW;
 Gr.PROP_STEP:=PROP_STEP;
 Gr.PROP_PROFILE:=PROP_PROFILE;
 Gr.PROP_WIDTH:=PROP_WIDTH;
 Gr.PROP_LENGTH:=PROP_LENGTH;
 End;
End;

Procedure TMaterial.UpdateNAME(OForm:TComponent);
Var
 nnn:TSpr1LabeledEdit;
Begin
 if (cparent<>3) then
 Begin
 NAME:=GetTextValueOfPointer(IntToStr(CPARENT),SchemaName+'MATERIALS','nrec','name',False);
 case cparent of
  4: Begin // Лист
 ...
 nnn.NAME:=NAME;
 if nnn.Edit(OForm,False,'Уточните наименование материала','Наименование',False) then NAME:=nnn.NAME;
 nnn.Free;
...
procedure TMaterialForm.UpdatePropList;
Var
// tempValue:int64;
 i,CTop:integer;
...
Function TMaterial.GetByNrec(vnrec:int64):boolean;
Var
 adoq:TAdoQuery;
Begin
 result:=False;
 ...
 adoq.SQL.Text:='select * from '+SchemaName+'MATERIALS'+' where nrec='+IntToStr(vnrec);
 adoq.Open;
...
Function TMaterial.GetByName(vname:string):boolean;
Var
 adoq:TAdoQuery;
Begin
 result:=False;
 ...
 adoq.SQL.Text:='select * from '+SchemaName+'MATERIALS'+' where NAME=:name';
 adoq.Open;
 if adoq.RecordCount=1 then result:=Read(adoq)
                       else Clear;
 adoq.Close;
...
procedure TMaterialForm.BitBtn1Click(Sender: TObject);
Var
 CGRMATERIALS:int64;
begin
 CGRMATERIALS:=LookupItemFromForm2(f_CGRMATERIALS.ItemIndex,GRADOQ,SchemaName+'GRMATERIALS');
 if CGRMATERIALS=0 then
  Begin
   ShowMessage('Обязательно укажите группу для материала!');
...
procedure TMaterialForm.CGRMATERIALSAppendBtnClick(Sender: TObject);
Var
 Mat:TMaterial;
 Gr:TGrMaterials;
begin
 ...
 If Mat.FromForm(Self) then Mat.CopyToGrMaterials(Gr);
 if Gr.Edit(Self,True) then
  Begin
  if Not GRADOQ.Locate('NAME',Gr.NAME,[loCaseInsensitive]) then
   Begin
    GRADOQ.Append;
    if Not Gr.Save(Self,GRADOQ) then GRADOQ.Cancel;
    GRADOQ.Requery;
    if GRADOQ.Locate('NAME',Gr.NAME,[loCaseInsensitive]) then
     LookupItemToForm(f_CGRMATERIALS,GRADOQ,GRADOQ['nrec'],'name',EmptyStr,EmptyStr,False);
   End;
  End;
 Mat.Free;
 Gr.Free;
...
procedure TMaterialForm.FormDestroy(Sender: TObject);
begin
...
procedure TMaterialForm.PROP_CARVTYPEAppendBtnClick(Sender: TObject);
Var
...
procedure TMaterialForm.PROP_CARVTYPEComboUpdateBtnClick(Sender: TObject);
Var
 tempValue:int64;
begin
...
procedure TMaterialForm.PROP_FIXTYPEAppendBtnClick(Sender: TObject);
Var
 Spr1LabeledEdit:TSpr1LabeledEdit;
begin
 Spr1LabeledEdit:=TSpr1LabeledEdit.Create;
...
procedure TMaterialForm.PROP_FIXTYPEComboUpdateBtnClick(Sender: TObject);
Var
 tempValue:int64;
begin
...
procedure TMaterialForm.PROP_LENGTHAppendBtnClick(Sender: TObject);
Var
...
procedure TMaterialForm.PROP_LENGTHComboUpdateBtnClick(Sender: TObject);
Var
 tempValue:int64;
begin
...
procedure TMaterialForm.PROP_NORMDOC0AppendBtnClick(Sender: TObject);
Var
 Spr1LabeledEdit:TSpr1LabeledEdit;
begin
 Spr1LabeledEdit:=TSpr1LabeledEdit.Create;
...
procedure TMaterialForm.PROP_NORMDOC0ComboUpdateBtnClick(Sender: TObject);
Var
 tempValue:int64;
begin
...
procedure TMaterialForm.PROP_NORMDOC1AppendBtnClick(Sender: TObject);
Var
 Spr1LabeledEdit:TSpr1LabeledEdit;
begin
 Spr1LabeledEdit:=TSpr1LabeledEdit.Create;
...
procedure TMaterialForm.PROP_PROFILEAppendBtnClick(Sender: TObject);
Var
 Spr1LabeledEdit:TSpr1LabeledEdit;
begin
 Spr1LabeledEdit:=TSpr1LabeledEdit.Create;
...
procedure TMaterialForm.PROP_PROFILEComboUpdateBtnClick(Sender: TObject);
Var
 tempValue:int64;
begin
...
procedure TMaterialForm.PROP_PROFTYPEAppendBtnClick(Sender: TObject);
Var
 Spr1LabeledEdit:TSpr1LabeledEdit;
begin
 Spr1LabeledEdit:=TSpr1LabeledEdit.Create;
...
procedure TMaterialForm.PROP_PROFTYPEComboUpdateBtnClick(Sender: TObject);
Var
 tempValue:int64;
begin
...
procedure TMaterialForm.PROP_STEELMARKAppendBtnClick(Sender: TObject);
Var
...
procedure TMaterialForm.PROP_STEELMARKComboUpdateBtnClick(Sender: TObject);
Var
 tempValue:int64;
begin
...
procedure TMaterialForm.PROP_THICKAppendBtnClick(Sender: TObject);
Var
 Spr1JvCalcEdit:TSpr1JvCalcEdit;
begin
 Spr1JvCalcEdit:=TSpr1JvCalcEdit.Create;
...
procedure TMaterialForm.PROP_THICKComboUpdateBtnClick(Sender: TObject);
Var
 tempValue:int64;
begin
...
procedure TMaterialForm.PROP_WIDTHAppendBtnClick(Sender: TObject);
Var
 Spr1JvCalcEdit:TSpr1JvCalcEdit;
begin
 Spr1JvCalcEdit:=TSpr1JvCalcEdit.Create;
...
procedure TMaterialForm.PROP_WIDTHComboUpdateBtnClick(Sender: TObject);
Var
 tempValue:int64;
begin
...
Function TMaterial.Save(OForm:TComponent;Var Dataset:TAdoQuery):boolean;
Var
 PropList:TStringList;
Begin
 result:=True;
 ...
 Dataset['CGRMATERIALS']:=CGRMATERIALS;
 Dataset['NAME']:=NAME;
...
Function TMaterial.Read(Var Dataset:TAdoQuery):boolean;
Var
 PropList:TStringList;
Begin
 result:=True;
...
Function TMaterial.Edit(OForm:TComponent;New:boolean=False):boolean;
Var
 Form: TMaterialForm;
begin
 Form:=TMaterialForm.Create(OForm);
...
Function TMaterial.ToForm(Var Form:TMaterialForm;New:boolean):boolean;
Var
 i:integer;
 orderby,wh:string;
Begin
 ...
 wh:='active=1 and cparent='+IntToStr(cparent);
 Form.GRADOQ.SQL.Text:='select * from '+SchemaName+'GRMATERIALS'+' where '+wh+orderby;
// ShowMessage(Form.GRADOQ.SQL.Text);
 Form.GRADOQ.Open;
// ShowMessage('2');

 LookupItemToForm(Form.f_CGRMATERIALS,Form.GRADOQ,CGRMATERIALS,'name',EmptyStr,EmptyStr,False);
...
Function TMaterial.FromForm(Var Form:TMaterialForm):boolean;
Begin
 nrec:=Form.f_nrec;
 ...
 CGRMATERIALS:=LookupItemFromForm2(Form.f_CGRMATERIALS.ItemIndex,Form.GRADOQ,SchemaName+'GRMATERIALS');
...
Procedure TMaterial.UpdatePROP_STEELMARK_C;
Begin
...
Procedure TMaterial.UpdatePROP_NORMDOC0_C;
Begin
...
Procedure TMaterial.UpdatePROP_NORMDOC1_C;
Begin
...
Procedure TMaterial.UpdatePROP_PROFTYPE_C;
Begin
...
Procedure TMaterial.UpdatePROP_FIXTYPE_C;
Begin
...
Procedure TMaterial.UpdatePROP_CARVTYPE_C;
Begin
...
Procedure TMaterial.UpdatePROP_PROFILE_C;
Begin
...
Procedure TMaterial.UpdatePROP_THICK_C;
Begin
...
Procedure TMaterial.UpdatePROP_WIDTH_C;
Begin
...
Procedure TMaterial.UpdatePROP_LENGTH_C;
Begin
...
Procedure TMaterial.PropsToForm(Form:TComponent);
Var
 c,cadoq:TComponent;
Begin
